...
> **BQ backend source:** the deployable SQL for the BigQuery objects the
> notebooks use is extracted verbatim into `backend/` so **git is the source of
> truth**. Backend-specific guidance lives in **`backend/CLAUDE.md`** (and
> `backend/README.md`); the v3 redesign is in `backend/REDESIGN.md`.
...
│   └── extract_backend.py  # Extract the BQ SQL → backend/*.sql (see backend/CLAUDE.md)
├── backend/                # Deployable BQ SQL source (git = truth); see backend/CLAUDE.md
