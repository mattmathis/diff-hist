CREATE TABLE FUNCTION `mlab-collaboration`.mm_preproduction.experimental_ndt7_isp_histograms(method STRING, xAxis STRING, binSize INT64, field STRING, startDate DATE, endDate DATE, siteRegex STRING)
AS
WITH 

# 202501GID L12 Create fcn experimental_ndt7_isp_histograms
# 
...
# SELECT * FROM `mlab-collaboration.mm_preproduction.experimental_ndt7_isp_histograms`('DS16', 'none', 50, 'MeanThroughputMbps', '2025-11-09', '2025-11-19', '.*');
  ...
  ndt7_all_sources AS (
    ...
    (SELECT * EXCEPT ( _internal202511, filter ), _internal202511.raw, 
    FROM `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1C`  -- Workround for DS1V problems, download only
    WHERE method LIKE '%DS1C%' AND field NOT LIKE '%upload%'
    )
          UNION ALL

    (SELECT * EXCEPT ( _internal202511, filter ), _internal202511.raw, 
    FROM `mlab-collaboration.mm_preproduction.extended_intermediate_downloads_DS1V`  -- Works poorly, BQ timeout
    WHERE method LIKE '%DS1V%' AND field NOT LIKE '%upload%'
    )
          UNION ALL

    (SELECT * EXCEPT ( _internal202511, filter ), _internal202511.raw,
    FROM `mlab-collaboration.mm_preproduction.extended_intermediate_uploads_DS1V`  -- Works poorly, BQ timeout
...
# Add metro, metro* stats and maxFingerprint(s)
