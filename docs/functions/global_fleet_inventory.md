# `global_fleet_inventory` (table function)

**BigQuery resource:** `mm_preproduction.global_fleet_inventory`  
...
_TODO (review): summarise what this function computes._
...
_Could not probe automatically; see the definition's final `SELECT` below._

## Call sites in this dashboard

- **panel #1 'Server egress value estimated from TCPinfo using ${costModel}  for ${duration} days ending ${endDate}'**
    - `startDate` = `DATE_SUB("$endDate", INTERVAL ${duration:raw}-1 DAY)`
